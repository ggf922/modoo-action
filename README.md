...
### 포인트 체계 (잔액P 폐지 → 경매P 통합)
| 포인트 | 컬럼 | 용도 | 충전 | 출금 |
|---|---|---|---|---|
| 경매 포인트 | `auctionPoint` | 경매 참가비 사용 + **미낙찰 보상 환급** + 충전 | ✅ | ❌ |
| 임금 포인트 | `wagePoint` | 추천 수당 적립 | ❌ | ✅ |
| 총 보유 포인트 | (합계) | 위 2종 합산 표시 | - | - |

> **포인트 흐름 예시**: 경매P 50,000 보유 → 30,000짜리 경매 참여(−30,000, 잔여 20,000) → **낙찰 시** 해당 상품 구매 확정 / **미낙찰 시** 미낙찰 보상(예: 700P)이 경매P로 환급되어 **50,700**처럼 다시 경매에 사용 가능.
> (`balancePoint` 컬럼은 DB에 남아있으나 더 이상 적립/표시하지 않음 — 레거시)
...
5. **출금**: **임금포인트(`wagePoint`)에서만** 차감(경매P는 경매·충전 전용으로 출금 불가), 최소 금액 검증, PENDING → 관리자 승인 시 차감 + COMPLETED
...
- `POST /api/admin/members/grant-vip-auction` (VIP 이상 등급 회원에게 경매P 강제 일괄 지급 — `{amount, reason}`, 대상: VIP·VVIP·대리점·총판·이사)
...
- **잔액포인트 → 경매포인트 통합**: 잔액P 카드/컬럼/필터를 전 화면에서 제거. **미낙찰 보상이 경매P로 환급**되어 즉시 재참여 가능(예: 50,000P → 30,000 경매 미낙찰 시 700P 환급 → 50,700P). 출금은 임금P만 가능
- **관리자 개별 포인트 발송**: 회원관리에서 아이디/이름/닉네임 검색 후 **포인트발송** 버튼으로 경매P/임금P를 개별 지급(회수 시 음수). 회원 목록에 **등급 인라인 드롭다운**으로 즉시 등급 변경(상세 모달의 "등급 적용"과 동일 기능)
- **조직도 등급 표시 + 색상 구분**: 회원/관리자 조직도 SVG 노드를 **등급별 색상**(일반 회색·VIP 주황·VVIP 노랑·대리점 파랑·총판 보라·이사 로즈)으로 테두리/뱃지 표시 + 범례 + 노드 상세에 등급 표기
- **VIP 이상 경매P 강제 일괄 지급**: 등급지급 화면 상단에서 VIP·VVIP·대리점·총판·이사 등급 회원 전원에게 경매P를 한 번에 강제 지급(대상 인원 표시 + 트랜잭션 + 내역 기록)
...
- **최종 업데이트**: 2026-06-12 (**포인트/등급 개편**: 잔액P→경매P 통합(미낙찰 보상 경매P 환급) · 관리자 개별 포인트 발송 · 목록 등급 인라인 변경 · 조직도 등급 색상 표시 · VIP 이상 경매P 강제 일괄 지급) / 회원 등급 시스템 6단계 · Batch G 배송관리·조직도 겹침 방지

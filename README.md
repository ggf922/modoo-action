...
3. **추천 보상(1회 지급 정책)**: 가입 시 추천코드 검증 → 신규회원 고유코드(8자리 대문자) 발급 + 추천관계 설정 (**가입 즉시에는 보상 미지급**)
   - **지급 시점**: 피추천자가 **VIP 이상 등급 + 활성** 상태가 되는 **최초 1회**에만 추천인 `auctionPoint += referralBonus` 지급 (`maybePayReferralReward`)
   - **트리거**: 관리자 등급 변경 / 활성·비활성 토글 / 회원 구독 활성화 시점에 조건 평가
   - **중복 방지**: 피추천자 `referralRewardPaid` 플래그로 1회만 지급 — **구독을 반복하거나 재승급/재활성화해도 재지급 없음**
   - **추천코드 미입력 시 회사(관리자)를 추천인으로 자동 적용** (조직도 단절 방지)
4. **조직도**: 본인 루트로 산하 최대 5단계 BFS 조회 (상위 추천인 절대 비노출)
   - **관리자 전체 조직도**: 회사(관리자) 루트 기준 전 회원 추천인 계보도 SVG 트리
   - **활성/비활성 표시**: VIP 이상 등급(일반회원 제외)의 노드에 활성(초록)·비활성(빨강) 상태 점 표시 + 상세 패널에 상태 배지 — 회원관리/구독관리의 `active` 상태와 동일 연동
5. **회원 관리(관리자)**: 정보 수정(이름/닉네임/이메일/연락처/추천인 변경 — 순환참조 방지) · 삭제(하위 회원은 삭제 회원의 추천인에게 자동 승계, 관리자 계정 삭제 차단) · **활성/비활성 토글**(회원목록 "상태" 열에서 1클릭 전환, 관리자 계정 비활성화 차단)
...
8. **구독 회원 관리(관리자)**: 구독료 납부 이력이 있거나 **VIP 이상 등급 + 활성 회원이 자동으로 구독관리 목록에 노출** — 활성 버튼으로 기간 연장 / 비활성화로 구독 종료
...
- `GET /api/admin/grant-history?limit&offset` (**지급 내역 보기** — 등급 일괄지급/월 구독료 수금은 배치 단위로 그룹핑, 개별 회원 지급/회수는 회원명과 함께 건별 반환. `total`·`hasMore` 페이지네이션 포함)
...
- **등급별 포인트 일괄 지급(관리자)**: 탭 `#/admin/grade-grant` — 등급별 회원 수 통계를 보여주고, **대상 등급·1인당 금액·사유** 입력 후 해당 등급 회원 전원에게 **경매P 일괄 지급**(`POST /api/admin/members/grade-grant`, 트랜잭션 + 회원별 포인트 내역 기록). 지급 인원 수 토스트 안내
  - **지급 내역 보기**: 우측 상단 버튼 → 지금까지 보낸 **등급 일괄지급 / 월 구독료 수금**(배치 단위, 인원·1인당·합계) + **개별 회원 지급/회수**(회원명 포함) 이력을 최근순 모달로 표시. 내역이 많아질 것을 대비해 **20건씩 '더 보기' 페이지네이션** 적용
- **충전 요청 관리(관리자)**: 탭 `#/admin/charges` — 회원의 충전 요청 승인/거절. **충전 내역 보기** 버튼 → 전체 요청/처리 이력(완료·거절·대기, 충전 완료 누적 합계)을 모달로 표시
...
- **폐쇄몰/도매몰 - 비로그인 가격 숨김**: 회원 전용 폐쇄몰 정책. **로그인하지 않은 방문자에게는 상품 가격을 노출하지 않음**. 상품 이미지·이름·카테고리·진행상태·참여현황은 그대로 보이되, **시중가/시작가/할인율(% OFF)만 "🔒 로그인 후 가격 확인"** 으로 대체하고 참여 버튼은 로그인·회원가입 안내로 전환. 화면뿐 아니라 **API 응답(`/api/products`, `/api/products/:id`)에서도 비로그인 요청 시 `startPrice`/`marketPrice`를 제거**(`priceHidden` 플래그 반환)하여 개발자도구 네트워크 탭으로도 가격을 볼 수 없음. 로그인 시 기존과 동일하게 전체 가격 노출

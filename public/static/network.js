...
async function pageNetwork() {
  ...
  allNodes.forEach(n => {
    ...
    // VIP 이상(일반회원 제외, 본인 제외)만 활성/비활성 상태 점 표시
    const showStatus = !isRoot && isVipOrAbove(n.grade)
    const statusDot = showStatus
      ? `<circle cx="14" cy="14" r="5" fill="${Number(n.active) === 0 ? '#ef4444' : '#22c55e'}" stroke="white" stroke-width="1.5"/>`
      : ''
    nodeEls += `<g transform="translate(${pos.x},${pos.y})" style="cursor:pointer"
      ...
      <text x="${showStatus ? 26 : 16}" y="24" font-size="14" font-weight="700" fill="#2D3748">${n.name}</text>
...
function showNodeDetail(n) {
  ...
  el.innerHTML = `
    ...
    <div class="space-y-2 text-sm">
      ...
      ${(!n.isRoot && isVipOrAbove(n.grade)) ? `<div class="flex justify-between py-2 border-b border-gray-50"><span class="text-gray-400">상태</span>${
        Number(n.active) === 0
